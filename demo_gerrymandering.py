#!/usr/bin/env python3
"""
Demo Gerrymandering Optimization
Quick demonstration of the library with smaller parameter sweep and GIF generation.
"""
...
class GerryConfig:
    ...
    steps: int = 500  # Reduced for demo
...
class FastGerryOptimizer:
    ...
    def calculate_score(self, districts):
        ...
        # Compactness score (simplified)
        compactness_score = 1.0  # Simplified for demo
    ...
    def optimize(self, save_history=True):
        ...
        for step in range(self.config.steps):
            ...
            # Store history
            if save_history and step % 25 == 0:  # More frequent for demo
...
class GerrymanderingAnalyzer:
    ...
    def create_simulation_gif(self, history, output_path="simulation.gif"):
        ...
        def animate(frame):
            ...
            ax1.set_title(f"Step {frame * 25}, Wins: {history['party_wins'][frame]}/{self.optimizer.config.target_districts}")
            
            # Plot optimization progress
            steps = np.arange(len(history['scores'])) * 25
...
def run_demo_parameter_sweep(data_path: str, output_dir: str = "results"):
    """Run a smaller parameter sweep for demonstration."""
    logger.info("Starting demo parameter sweep...")
    
    # Smaller parameter combinations for demo
    temperatures = [1000, 2000]
    cooling_rates = [0.99, 0.995]
    steps_list = [500]
    partisan_weights = [1.0, 2.0, 5.0]
    ...
    for temp, cool, steps, p_weight, target_party in product(temperatures, cooling_rates, steps_list, partisan_weights, target_parties):
        ...
        # Run multiple trials
        trial_results = []
        for trial in range(2):  # 2 trials per combination for demo
    ...
    results_df.to_csv(f"{output_dir}/demo_parameter_sweep_results.csv", index=False)
    
    # Create analysis plots
    create_demo_analysis_plots(results_df, output_dir)
    
    logger.info("Demo parameter sweep complete!")
    return results_df

def create_demo_analysis_plots(results_df: pd.DataFrame, output_dir: str):
    """Create analysis plots for demo parameter sweep results."""
    logger.info("Creating demo analysis plots...")
    
    # Set up plotting style
    plt.style.use('default')
    
    # Create a simple plot showing partisan weight effect
    ...
    for target_party in [0, 1]:
        ...
        ax.plot(party_data['partisan_weight'], party_data['avg_win_rate'], 
               marker='o', label=party_name, linewidth=2, markersize=8)
    
    ax.set_xlabel('Partisan Weight', fontsize=12)
    ax.set_ylabel('Average Win Rate', fontsize=12)
    ax.set_title('Effect of Partisan Weight on Win Rate', fontsize=14)
    ax.legend(fontsize=12)
    ax.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(f"{output_dir}/demo_parameter_analysis.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    ax.set_title('Best Parameters for Each Target Party', fontsize=14)
    ax.set_ylabel('Parameters', fontsize=12)
    ax.set_xlabel('Target Party (0=Center-Left, 1=Center-Right)', fontsize=12)
    
    plt.tight_layout()
    plt.savefig(f"{output_dir}/demo_best_parameters_heatmap.png", dpi=300, bbox_inches='tight')
    plt.close()
    
    logger.info(f"Demo analysis plots saved to {output_dir}")
...
if __name__ == "__main__":
    ...
    print("Running demo gerrymandering optimization...")
    
    # Run demo parameter sweep
    results = run_demo_parameter_sweep(data_path)
